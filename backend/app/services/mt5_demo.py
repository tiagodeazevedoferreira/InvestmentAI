...
class MetaTrader5DemoGateway:
    ...
    def __init__(self, *, login: int | None = None, server: str | None = None) -> None:
        self.login = login
        self.server = server
...
class MT5DemoBroker:
    ...
    The adapter is deliberately dependency-injected so CI can test broker
    semantics without installing or connecting to MetaTrader 5.
    """
    ...
    def _ensure_demo_account(self) -> Any:
        ...
        # MT5 constants are intentionally compared by semantic value when
        # available; an explicit live-looking server is always rejected above.
    ...
    def submit(self, intent: OrderIntent) -> dict[str, Any]:
        ...
        if mt5 is None:
            # Injected gateways may expose semantic constants directly.
        ...
        request = {
            ...
            "price": float(intent.limit_price if intent.limit_price is not None else tick),
            "deviation": 20,
    ...
    def _position(self, row: Any) -> dict[str, Any]:
        return {
            "position_id": str(self._value(row, "ticket", "")),
            "symbol": str(self._value(row, "symbol", "")).upper(),
            "quantity": float(self._value(row, "volume", 0.0)),
            "side": "BUY" if self._value(row, "type", 0) == 0 else "SELL",
            "price_open": float(self._value(row, "price_open", 0.0)),
        }

    def _order(self, row: Any) -> dict[str, Any]:
        return {
            "order_id": str(self._value(row, "ticket", "")),
            "symbol": str(self._value(row, "symbol", "")).upper(),
            "quantity": float(self._value(row, "volume_current", self._value(row, "volume_initial", 0.0))),
            "type": self._value(row, "type", None),
            "status": self._value(row, "state", None),
        }

    def _deal(self, row: Any) -> dict[str, Any]:
        return {
            "execution_id": str(self._value(row, "ticket", "")),
            "order_id": str(self._value(row, "order", "")),
            "symbol": str(self._value(row, "symbol", "")).upper(),
            "quantity": float(self._value(row, "volume", 0.0)),
            "price": float(self._value(row, "price", 0.0)),
            "time": self._value(row, "time", None),
        }
