...
class MetaTrader5DemoBroker:
    ...
    This adapter is the only layer allowed to call MT5 ``order_send`` for DEMO.
    It requires both an expected login and expected server; neither may be
    omitted. No password is handled by the application.
    """
    ...
    def connect(self) -> MT5DemoAccount:
        ...
        if login != self.expected_login:
            ...
            raise MT5DemoExecutionError(
                f"DEMO account mismatch: {login}; expected {self.expected_login}"
            )
        if server != self.expected_server:
            self.close()
            raise MT5DemoExecutionError(
                f"DEMO server mismatch: {server!r}; expected {self.expected_server!r}"
            )
    ...
    def submit(self, intent: OrderIntent) -> dict:
        ...
        done = self._value(result, "volume", 0.0)
        return {
            "accepted": retcode in {
            ...
            "retcode": retcode,
            "order": self._value(result, "order", 0),
            "deal": self._value(result, "deal", 0),
            "volume": float(done or 0.0),
            "symbol": symbol,
            "side": intent.side,
