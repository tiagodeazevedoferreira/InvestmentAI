...
## ADR-030 — Cross-asset diagnosis is descriptive and causally bounded
The pooled-vs-asset-specific ML experiment must emit a dedicated diagnostic report before any model optimization decision. The report includes class balance, prediction/probability bias, Brier score, ECE, first-vs-last fold degradation and a reference-vs-OOS feature-distribution shift summary for each asset/model. The feature reference must precede the first OOS timestamp. These diagnostics are observability evidence, not promotion criteria by themselves, and must not alter thresholds, execution policy, risk gates or broker authority. Real-data results remain research-only until explicitly reviewed.
