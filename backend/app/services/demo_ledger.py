...
class DemoOrderLedger:
    ...
    The ledger is intentionally independent from MT5 and the application's
    portfolio ledger. It records operational intent and execution evidence;
    it does not decide whether an order is allowed to execute.
    """
    ...
    def transition(
        ...
        *,
        ...
        if new_state not in self.VALID_STATES:
            raise DemoLedgerError(f"invalid DEMO order state: {new_state}")
        ...
        with self._connect() as connection:
            connection.execute(
                """UPDATE demo_orders
                   SET state=?, updated_at=?, order_id=COALESCE(?,order_id),
    ...
    def get(self, intent_id: str) -> DemoOrderRecord:
        ...
        if row is None:
            raise DemoLedgerError(f"unknown DEMO intent_id: {intent_id}")
    ...
    def pending(self) -> tuple[DemoOrderRecord, ...]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT intent_id FROM demo_orders WHERE state NOT IN ('FILLED','REJECTED','FAILED') ORDER BY created_at"
            ).fetchall()
